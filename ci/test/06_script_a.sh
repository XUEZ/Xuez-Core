...
export P_CI_DIR="${BASE_BUILD_DIR}/xuez-$HOST"
...
if [[ ${USE_MEMORY_SANITIZER} == "true" ]]; then
  ...
  # details.
  DOCKER_EXEC 'grep -v HAVE_SYS_GETRANDOM src/config/xuez-config.h > src/config/xuez-config.h.tmp && mv src/config/xuez-config.h.tmp src/config/xuez-config.h'
fi
