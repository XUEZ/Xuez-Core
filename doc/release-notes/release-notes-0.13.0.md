XUEZ Core version 0.13.0 is now available from:

  <https://xuez.org/bin/xuez-core-0.13.0/>
...
- Adding `dbcache=100` in xuez.conf
...
For example:
    ...
    $ src/xuez-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The XUEZ Core code base has
...
HD wallets are incompatible with older versions of XUEZ Core.
...
proposed parameters, a future release of XUEZ Core is expected that
...
The two phases are distinct in the XUEZ-Qt GUI. During the first one,
...
- #7550 `8b958ab` Input-from-stdin mode for xuez-cli (laanwj)
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use XUEZ/kB (MarcoFalke)
...
- #7692 `29b2be6` Remove P2P alert system (xuezdrak)
...
- #8275 `042c323` Remove bad chain alert partition check (xuezdrak)
...
- #7619 `18b3f1b` Add missing sudo entry in gitian VM setup (xuezdrak)
- #7616 `639ec58`  [depends] Delete unused patches  (MarcoFalke)
- #7658 `c15eb28` Add curl to Gitian setup instructions (xuezdrak)
...
- #7613 `3798e5d` Add autocomplete to xuez-qt's console window (GamerSg)
...
- #7636 `fb0ac48` Add xuez address label to request payment QR code (makevoid)
- #8231 `760a6c7` Fix a bug where the SplashScreen will not be hidden during startup (jonasschnelli)
- #8256 `af2421c` BUG: xuez-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from xuezd (sipa)
- #8288 `91abb77` Network-specific example address (laanwj)
- #7707 `a914968` UI support for abandoned transactions (jonasschnelli)
- #8207 `f7a403b` Add a link to the XUEZ-Core repository and website to the About Dialog (MarcoFalke)
...
- #7531 `18cb2d5` Add bip68-sequence.py to extended rpc tests (xuezdrak)
...
- #7773 `7c80e72` Fix comments in tests (xuezdrak)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for xuezd procs (MarcoFalke)
...
- #8164 `0f24eaf` [XUEZ-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of xuezd startup (laanwj)
...
- #7975 `fc23fee` Update xuez-core GitHub links (MarcoFalke)
...
- #7540 `ff46dd4` Rename OP_NOP3 to OP_CHECKSEQUENCEVERIFY (xuezdrak)
