XUEZ Core version 0.12.0 is now available from:

  <https://xuez.org/bin/xuez-core-0.12.0/>
...
backwards-compatible with pre-0.10 versions of XUEZ Core or other software:
...
pre-0.12 versions of XUEZ Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start XUEZ Core version 0.11 or
earlier.
...
ECDSA signatures inside XUEZ transactions now use validation using
...
Previous versions of XUEZ Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of XUEZ Core was to raise the default minimum
relay fee.

XUEZ Core 0.12 will have a strict maximum size on the mempool. The
...
XUEZ Core 0.12 also introduces new default policy limits on the length and
...
XUEZ Core 0.12 nodes. XUEZ Core will only allow replacement of
...
Note that the wallet in XUEZ Core 0.12 does not yet have support for
...
This allows running xuezd without having to do any manual configuration.
...
XUEZ Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. XUEZ Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In XUEZ Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in XUEZ Core 0.13, and it is at this time undecided whether the
...
XUEZ Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
XUEZ Core automatically creates a hidden service to listen on, without
manual configuration. XUEZ Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if XUEZ Core is listening, and
...
XUEZd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals XUEZ
Core to use floating fees. By default, XUEZ Core will use floating
fees.
...
(default: `0.0002` XUEZ/kB).

At all times, XUEZ Core will cap fees at `-maxtxfee=<x>` (default:
0.10) XUEZ.
Furthermore, XUEZ Core will never create transactions paying less than
...
`xuez.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /xuezrpc>
        ...
        # optional bypass xuezd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< xuezrpc:<password>
    </Location>
...
Restarting xuezd will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of XUEZ-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new xuez-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include xuez-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix xuez-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of xuez-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [xuez-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add BITCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libxuezconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "XUEZ" (Ross Nicoll)
