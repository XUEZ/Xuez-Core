XUEZ Core version 0.14.0 is now available from:

  <https://xuez.org/bin/xuez-core-0.14.0/>
...
XUEZ Core has supported automatically pruning the blockchain since 0.11. Pruning
...
When XUEZ Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `xuez-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`xuez-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/xuez-cli -named help command="help"
    src/xuez-cli -named getblockhash height=0
    src/xuez-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/xuez-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert System was [disabled and deprecated](https://xuez.org/en/alert/2016-11-01-alert-retirement) in XUEZ Core 0.12.1 and removed in 0.13.0. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of XUEZ Core. This was added in conjunction with
...
- #8722 `bb843ad` xuez-cli: More detailed error reporting (laanwj)
...
- #7562 `1eef038` Bump transaction version default to 2 (xuezdrak)
...
- #8608 `7e9ab95` Install manpages via make install, also add some autogenerated manpages (nomnomxuez)
...
- #8568 `078900d` new var `DIST_CONTRIB` adds useful things for packagers from contrib (nomnomxuez)
...
- #8830 `ef0801b` test: Add option to run xuez-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to xuez-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to xuez-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and xuez-util-test.py (jnewbery)
...
- #9206 `e662d28` Make test constant consistent with consensus.h (xuezdrak)
...
- #9691 `fc67cd2` Init ECC context for `test_xuez_fuzzy` (gmaxwell)
...
- #9865 `289204f` Change xuez address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` xuezd: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow xuez-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` xuez-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to xuez-tx (jnewbery)
