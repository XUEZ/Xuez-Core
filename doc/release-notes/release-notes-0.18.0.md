XUEZ Core version 0.18.0 is now available from:
...
`/Applications/XUEZ-Qt` (on Mac) or `xuezd`/`xuez-qt` (on
Linux).
...
recommended to use XUEZ Core on unsupported systems.

XUEZ Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, XUEZ Core does not yet change appearance when
...
`contrib/init/xuezd.service`, it has been changed to use
`/var/lib/xuezd` as the data directory instead of
`~xuez/.xuez`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/xuezd` will
exist has enough space (check using `df -h /var/lib/xuezd`), and
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the XUEZ Core
...
The systemd init file (`contrib/init/xuezd.service`) has been changed
to use `/var/lib/xuezd` as the data directory instead of
`~xuez/.xuez`. This change makes XUEZ Core more consistent with
...
`datadir` option in `/etc/xuez/xuez.conf` will have no effect.
...
- A new [document](https://github.com/bitcoin/bitcoin/blob/master/doc/bitcoin-conf.md)
  about the `xuez.conf` file describes how to use it to configure
  XUEZ Core.

- A new document introduces XUEZ Core's BIP174 [Partially-Signed
  XUEZ Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent XUEZ-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  XUEZ Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when XUEZ Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of XUEZ Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart xuezd with the `-deprecatedrpc=generate`
...
- A new `xuez-wallet` tool is now distributed alongside XUEZ
...
This section describes planned changes to XUEZ Core that may affect
other XUEZ software and services.

- Since version 0.16.0, XUEZ Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with XUEZ Core 0.20 (expected about
  a year after 0.18), XUEZ Core will default to native segwit
  ...
  to bech32 addresses, and if the XUEZ Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in XUEZ Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any XUEZ Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, XUEZ Core
...
- A sub-project of XUEZ Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with XUEZ Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to XUEZ Core's own implementation, although entropy
  gathered by XUEZ Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves XUEZ Core a
...
- On macOS, XUEZ Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `xuez_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libxuez_server.a` (jnewbery)
...
- #13926 [Tools] xuez-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like xuezd (MarcoFalke)
...
- #14521 qt, docs: Fix `xuez-qt -version` output formatting (hebasto)
...
- #11625 Add XUEZApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libxuez_server` (laanwj)
...
- #15154 Configure: xuez-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/xuez-wallet (Sjors)
...
- #11526 Visual Studio build configuration for XUEZ Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libxuez_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in xuez-util-test.py (isghe)
...
- #12775 Integration of property based testing into XUEZ Core (Christewart)
...
- #12246 Bugfix: Only run xuez-tx tests when xuez-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_xuez` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_xuez` (MarcoFalke)
...
- #14186 xuez-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_xuez.conf` message on startup if conf file exists (leishman)
...
- #12255 Update xuez.service to conform to init.md (dongcarl)
...
- #14497 Add doc/xuez-conf.md (hebasto)
...
- #15354 Add missing `xuez-wallet` tool manpages (MarcoFalke)
...
- #15765 correct xuezconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support xuez folder on OSX (JimmyMow)
