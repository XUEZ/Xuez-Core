...
XUEZ Core version 0.20.0 is now available from:
...
#### Removal of BIP61 reject network messages from XUEZ Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since XUEZ Core version 0.18.0.
...
- Testing or debugging of implementations of the XUEZ P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of XUEZ Core. XUEZ Core logs debug messages
...
- OpenSSL is no longer used by XUEZ Core.  (#17265)

- BIP70 support has been fully removed from XUEZ Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  `xuezd help` for more information.  This option is experimental and
...
- All custom settings configured when XUEZ Core starts are now
...
- The "Start XUEZ Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple XUEZ Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed XUEZ Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitcoin-core/HWI) for
  signing.  Future versions of XUEZ Core should support a GUI option
...
- XUEZ Core's automatically-generated source code documentation is
...
- The `xuez-cli` utility used with the `-getinfo` parameter now
...
- The `xuez-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `xuezstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_xuez-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_xuez` binary (hebasto)
...
- #18702 Fix ASLR for xuez-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate xuez schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest xuez.conf (sdaftuar)
...
- #18022 Fix appveyor `test_xuez` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (xuez-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone xuez-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about XUEZ Core files (hebasto)
...
- #17119 Fix broken xuez-cli examples (andrewtoth)
...
- #17503 Remove bitness from xuez-qt help message and manpage (laanwj)
...
- #17648 Rename wallet-tool references to xuez-wallet (hel-o)
...
- #18341 Replace remaining literal XUEZ with `CURRENCY_UNIT` (domob1812)
...
- #16115 On xuezd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from xuez-cli -getinfo (malevolent)
