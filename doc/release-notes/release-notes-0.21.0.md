...
XUEZ Core version 0.21.0 is now available from:
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  Version 2 hidden services are still fully supported by XUEZ Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. XUEZ Core will no longer
...
- A new `xuez-cli -netinfo` command provides a network peer connections
...
- A new `xuez-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `xuez-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `xuez-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when XUEZ Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `xuez-wallet` tool which performs the salvage
...
XUEZ Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`xuez.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
XUEZ Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `xuez-cli createwallet` or `xuez-wallet
...
Users who already have a XUEZ Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of XUEZ Core.
...
The change to using descriptors changes the default derivation paths used by XUEZ Core
...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in XUEZ per 1,000
  vbytes (XUEZ/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from XUEZ/kB
  to sat/vB and the help documentation in XUEZ/kB is updated to XUEZ/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from XUEZ/kvB to sat/vB and the
  ...
  difference between XUEZ/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in XUEZ/kvB rather than sat/vB should raise an
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time XUEZ Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `xuez.conf`
...
- #19284 Add seed.xuez.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in xuez-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add xuez-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in xuez-cli (laanwj)
...
### Build system
- #18504 Drop xuez-tx and xuez-wallet dependencies on libevent (ryanofsky)
...
- #18653 add coverage for xuez-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz XUEZ Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitrexuez)
...
- #18817 Document differences in xuezd and xuez-qt locale handling (practicalswift)
