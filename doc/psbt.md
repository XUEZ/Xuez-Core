# PSBT Howto for XUEZ Core

Since XUEZ Core 0.17, an RPC interface exists for Partially Signed XUEZ
...
PSBT is an interchange format for XUEZ transactions that are not fully signed
...
#### Multisig with multiple XUEZ Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
XUEZ Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `xuez-cli` is used.
...
Later, when *V* XUEZ has been received on *Amulti*, and Bob and Carol want to
