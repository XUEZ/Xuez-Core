...
The XUEZ Core daemon can be configured to act as a trusted "border
router", implementing the xuez wire protocol and relay, making
...
The ZeroMQ feature in XUEZ Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building xuezd:
...
For instance:
    ...
    $ xuezd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/xuezd.tx.raw \
...
These options can also be provided in xuez.conf.
...
From the perspective of xuezd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into xuezd directly. Furthermore, no information is
...
using. XUEZd appends an up-counting sequence number to each
