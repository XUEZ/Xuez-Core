...
- [Development guidelines](#development-guidelines)
    - [General XUEZ Core](#general-xuez-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
XUEZ Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play xuezs" on the test network, if you
...
XUEZ Core is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_xuez
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_xuez --log_level=test_suite
$ valgrind -v --leak-check=full src/xuezd -printtoconsole
...
# A coverage report will now be accessible at `./test_xuez.coverage/index.html`.
```
...
To profile a running xuezd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep xuezd` -- sleep 60
```
...
XUEZ Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`bitcoind`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `xuezd.cpp`. Responsible for starting up and
...
However, in open source software such as XUEZ Core, where everyone uses
...
pay attention to for reviewers of XUEZ Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: XUEZ Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
be sent upstream, but bugfixes may also be prudent to PR against XUEZ Core so that they can be integrated
...
file descriptors, it will cause problems with XUEZ's `select()` loop, because
...
compatibility between nodes. This happened in XUEZ 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to XUEZ's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `xuez-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `xuez-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of XUEZ Core, the wallet was always
