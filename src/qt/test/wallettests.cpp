...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<XUEZAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_xuez-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_xuez-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_xuez-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = XUEZUnits::formatWithUnit(unit, balance, false, XUEZUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    XUEZAmountField* amountInput = receiveCoinsDialog.findChild<XUEZAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_xuez-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
