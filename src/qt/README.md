...
Represents the main window of the XUEZ UI.
...
ClientModel is used by the main application `xuezgui` and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a xuez:... link from a browser).
...
* UI elements like XUEZAmountField, which inherit from QWidget.
* `xuezstrings.cpp`: automatically generated
* `xuezunits.(h/cpp)`: XUEZ / mXUEZ / etc handling
* `callback.h`
...
* don't change `local/xuez_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "xuez-qt" under "Run", which is where you can also add command line arguments)
