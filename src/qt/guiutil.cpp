...
#include <qt/xuezaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a XUEZ address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new XUEZAddressEntryValidator(parent));
    widget->setCheckValidator(new XUEZAddressCheckValidator(parent));
}

bool parseXUEZURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no xuez: URI
    if(!uri.isValid() || uri.scheme() != QString("xuez"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!XUEZUnits::parse(XUEZUnits::XUEZ, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseXUEZURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseXUEZURI(uriInstance, out);
}

QString formatXUEZURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("xuez:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(XUEZUnits::format(XUEZUnits::XUEZ, info.amount, false, XUEZUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openXUEZConf()
{
    ...
    /* Open xuez.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "XUEZ.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "XUEZ (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("XUEZ (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "xuez.desktop";
    return GetAutostartDir() / strprintf("xuez-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a xuez.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=XUEZ\n";
        else
            optionFile << strprintf("Name=XUEZ (%s)\n", chain);
