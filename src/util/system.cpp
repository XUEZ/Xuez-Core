...
const char * const BITCOIN_CONF_FILENAME = "xuez.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //xuez-tx using stdin
        std::string val;
...
{
    // Windows: C:\Users\Username\AppData\Roaming\XUEZ
    // macOS: ~/Library/Application Support/XUEZ
    // Unix-like: ~/.xuez
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XUEZ";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/XUEZ";
#else
    // Unix-like
    return pathRet / ".xuez";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("ComputerCraftr") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "John \"ComputerCraftr\" Studnicka";
    }
