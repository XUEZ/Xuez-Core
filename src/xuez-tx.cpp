...
static void SetupXUEZTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " xuez-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  xuez-tx [options] <hex-tx> [commands]  Update hex-encoded xuez transaction\n" +
            "or:     xuez-tx [options] -create [commands]   Create hex-encoded xuez transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded xuez transaction
