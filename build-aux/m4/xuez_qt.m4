...
dnl Output: If qt version is auto, set xuez_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$xuez_qt_want_version" = xauto && test "x$xuez_qt_force" != xyes; then
    if test "x$xuez_enable_qt" != xno; then
      AC_MSG_WARN([$1; xuez-qt frontend will not be built])
    fi
    xuez_enable_qt=no
    xuez_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$xuez_enable_qt" != xno && test "x$xuez_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build xuez-qt GUI (default=auto)])],
    [
     xuez_qt_want_version=$withval
     if test "x$xuez_qt_want_version" = xyes; then
       xuez_qt_force=yes
       xuez_qt_want_version=auto
     fi
    ],
    [xuez_qt_want_version=auto])
...
dnl Outputs: xuez_enable_qt, xuez_enable_qt_dbus, xuez_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "x$xuez_cv_static_qt" = xyes; then
  ...
  BITCOIN_QT_CHECK([
    xuez_enable_qt=yes
    xuez_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      xuez_enable_qt_test=no
    fi
    xuez_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      xuez_enable_qt_dbus=yes
    fi
  ...
  if test x$xuez_enable_qt = xyes; then
    AC_MSG_RESULT([$xuez_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$xuez_enable_qt])
  fi
...
dnl Output: xuez_cv_static_qt=yes|no
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, xuez_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [xuez_cv_static_qt=yes],
      [xuez_cv_static_qt=no])
    ])
])
