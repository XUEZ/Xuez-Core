...
## run with -
##  bash contrib/build-docker-scripts/buildWin_32.sh
...
export DOCKER_PACKAGES=${DOCKER_PACKAGES:-build-essential libtool autotools-dev bison automake pkg-config bsdmainutils curl ca-certificates ccache python3 rsync git procps nsis g++-mingw-w64-x86-64 g++-mingw-w64-i686 wine-binfmt wine64 file}
...
COPYBINS () {
  ...
  DOCKER_EXEC cp $BASE_ROOT_DIR/xuez*setup.exe /xueztmp/xuez-$VER-win32-setup.exe
...
DOCKER_EXEC update-alternatives --set i686-w64-mingw32-g++ \$\(which i686-w64-mingw32-g++-posix\)
